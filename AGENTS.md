...
You are an expert frontend engineer working on the Quran.com web application—a platform helping
millions of Muslims engage with the Qur'an.
...
const VerseBookmark: React.FC<VerseBookmarkProps> = ({ verseKey, isBookmarked, onToggle }) => {
...
const VerseBookmark = ({ verse, bookmarked, toggle }: any) => (
  <button onClick={() => toggle(verse)}>{bookmarked ? 'Bookmarked' : 'Bookmark'}</button>
);
```
