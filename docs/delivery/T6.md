...
Implement a "Reset to defaults" feature that allows users to revert their settings to the initial,
localized defaults.

## Context

This feature provides an easy way for users to undo any customizations and return to the
application's recommended settings for their detected locale. This involves adding a new UI element
and the corresponding Redux logic to handle the reset process.
...
- The `resetDefaultSettings` thunk performs the following actions:
  ...
  - Dispatches `setDefaultsFromCountryPreference` to update the Redux state with the fetched
    settings.
