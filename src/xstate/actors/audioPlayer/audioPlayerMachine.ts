...
const initialSavedState = getStateFromLocalStorage();

const audioPlayerMachine =
...
export const audioPlayerService = interpret(audioPlayerMachine, { devTools: true })
  .start()
  .onTransition((state, event) => {
    persistXstateContext(event);
  });
// TODO: group multiple actions that assign a single value into one
// TODO: add global listener that would listen to context changes and convert it to the local storage storable value.
