...
import useCountRangeNotes from './useCountRangeNotes';
...
/**
 * Hook that fetches note counts in batches of 20 verses to optimize API calls.
 ...
 * @returns {BatchedCountRangeNotesResponse} Object containing the note count, loading state, and error
 */
const useBatchedCountRangeNotes = (verseKey: string): BatchedCountRangeNotesResponse => {
  ...
  const { data, isLoading, error } = useCountRangeNotes(batchRange);
...
export default useBatchedCountRangeNotes;
