...
import { countQuestionsWithinRange, QuestionsCount } from '@/utils/auth/api';
...
type CountRangeQuestionsResponse = {
  data: Record<string, QuestionsCount>;
  isLoading: boolean;
  error: Error | null;
};
...
const useCountRangeQuestions = (questionsRange: Range): CountRangeQuestionsResponse => {
  ...
  const { data, isValidating, error } = useSWRImmutable<Record<string, QuestionsCount>>(
    questionsRange
      ? makeCountQuestionsWithinRangeUrl(questionsRange.from, questionsRange.to, lang as Language)
      : null,
    async (): Promise<Record<string, QuestionsCount>> => {
