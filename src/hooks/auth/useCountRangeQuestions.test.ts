...
// Type for malformed API responses where type keys may have incorrect casing
type MalformedQuestionsData = Record<string, { types: Record<string, number>; total: number }>;
...
describe('normalizeQuestionsData', () => {
  it('should normalize lowercase first letter type keys to uppercase', () => {
    ...
    const input: MalformedQuestionsData = {
      '2:6': {
        types: {
          cLARIFICATION: 2,
          tAFSIR: 1,
    ...
    const result = normalizeQuestionsData(input as Record<string, QuestionsData>);
