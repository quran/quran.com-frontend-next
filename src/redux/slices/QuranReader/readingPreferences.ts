...
export const readingPreferencesSlice = createSlice({
  ...
  reducers: {
    ...
    setReflectionLanguages: (state, action: PayloadAction<string[]>) => {
      // Note: action creators generated by RTK don't type `meta` unless using `prepare`.
      // We still allow thunks/callers to dispatch an action object with `meta`.
      const skipCustomization =
        (action as { meta?: { skipCustomization?: boolean } }).meta?.skipCustomization === true;
      return {
        ...state,
