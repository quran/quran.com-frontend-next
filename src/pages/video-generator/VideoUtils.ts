...
export function getBackgroundWithOpacity(colorObj, newAlpha) {
  ...
  const modifiedString = colorString.replace(rgbaRegex, (_match, red, green, blue, alpha) => {
...
export function getVideosArray() {
  const flattenObject = (obj) => {
    ...
    for (const [key, value] of Object.entries(obj)) {
      result.push({ ...value, id: parseInt(key) }) 
    }
...
export function validateVerseRange(from = 1, to, versesCount) {
  to = to || versesCount;
  return from <= to && from <= versesCount && to <= versesCount;
}
